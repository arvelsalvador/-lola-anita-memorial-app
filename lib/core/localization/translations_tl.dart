/// Tagalog translations for the Nita memorial app.
/// Keep this file's key set in sync with translations_en.dart and
/// translations_bi.dart, run LanguageProvider.debugCheckTranslationKeysMatch()
/// after any edit to catch drift.
const String _address = 'Purok 3, Barangay 7, Mercedes, Camarines Norte';

const Map<String, String> translationsTl = {
  'app_title': 'Sa Mahal na Alaala',
  'nav_home': 'Tahanan',
  'nav_gallery': 'Galeri',
  'nav_family': 'Pamilya',
  'nav_tribute': 'Pagkilala',
  'nav_words': 'Mga Salita',
  'nav_favorites': 'Pakikiramay',
  'splash_quote':
      'Ang mga mahal natin sa buhay ay hindi nawawala,\nSila ay laging kasama natin araw-araw.',
  'splash_subtitle': 'SA MAHAL NA ALAALA',
  'splash_tap': 'Pindutin upang pumasok',
  'hero_tagline': 'Minamahal na lola, tagapagkuwento ng mga alaala',
  'section_her_words': 'Kanyang mga salita',
  'section_her_journey': 'Kanyang paglalakbay',
  'section_about_her': 'Tungkol sa kanya',
  'section_cherished_memories': 'Mga Ala-Ala',
  'section_final_tribute': 'Huling pagkilala',
  'section_words_family': 'Mga salita ng pamilya',
  'condolences_subtitle': 'Mag-Sindihan ng kandila para kay Nanay',
  'family_tree_tap_hint': 'I-tap ang pangalan para makita ang mga alaala',
  'family_tree_unlinked': 'Iba pang mga Apo',
  'settings_language': 'Wika',
  'settings_title': 'Mga Setting',
  'settings_subtitle': 'I-personalize ang iyong karanasan',
  'settings_desc_dev': 'Alamin ang tungkol sa developer ng app na ito.',
  'settings_desc_about': 'Tingnan ang aming misyon at mga layunin.',
  'settings_desc_contact':
      'Ipadala sa amin ang iyong mga katanungan o suhestiyon.',
  'settings_empty': 'Wala pang laman dito',
  'settings_english': 'Ingles',
  'settings_tagalog': 'Tagalog',
  'settings_bicol': 'Bikol',
  'settings_about_app_title': 'Tungkol sa Amin',
  'settings_about_app_body':
      'Isang permanenteng digital na alaala para kay Lola Anita Daiz Lumbao — ang kanyang kwento, pamilya, at mga larawan, na ingatan nang may pagmamahal.',
  'settings_about_app_version': 'Bersyon 1.0.0+1',
  'settings_about_app_credits':
      'Mga larawan ng pamilya · Kiss the Rain – Yiruma · Pinagsasaluhang kandila sa Firebase, gumagana kahit offline',
  'settings_about_why_title': 'Bakit namin ito ginawa?',
  'settings_about_why_body':
      'Gusto namin na manatili ang mga alaala ni Nanay Anita para sa susunod '
      'na mga henerasyon. Sa pamamagitan ng simpleng digital na espasyong '
      'ito, maipagpapatuloy ang kanyang kwento, ang pagmamahal ng aming '
      'pamilya, at ang mga sandaling mahalaga sa kanya.',
  'settings_about_memories_title': 'Mga Alaala',
  'settings_about_memories_body':
      'Mga kwento, karanasan, at mahahalagang pangyayari sa buhay ni Nanay.',
  'settings_about_photos_title': 'Mga Larawan',
  'settings_about_photos_body':
      'Mga larawan ng pamilya, mga mahal na mukha, at mga espesyal na sandali.',
  'settings_about_messages_title': 'Mga Mensahe',
  'settings_about_messages_body':
      'Mga mensahe ng pagmamahal at alaala mula sa pamilya at mga mahal sa kanya.',
  'settings_about_made_title': 'Ginawa nang may pagmamahal',
  'settings_about_made_body': 'Para kay Nanay, para sa pamilya.',
  'settings_about_thanks': 'Salamat sa pag-alala kasama namin.',
  'settings_about_dev_title': 'Tungkol sa Developer',
  'settings_about_dev_role': 'App Developer at Apo',
  'settings_about_dev_dedication':
      'Si Arvel Salvador ay anak ni Lorie Salvador at apo ni Nanay. Ang '
      'techie ng pamilya: mausisa, masikap, at puno ng ideya. Siya ang '
      'nagdisenyo at bumuo ng memorial na ito upang manatiling buhay ang '
      'kanyang kwento, pamilya, at mga larawan para sa mga susunod na '
      'henerasyon. Pagkatapos ng kolehiyo, patuloy siyang naglilingkod sa '
      'pamilya gamit ang kanyang mga talento. Ang app na ito ay alay niya '
      'pabalik kay Lola. Ginawa nang may pagmamahal para kay Nanay.',
  'settings_about_dev_view_family': 'Tingnan sa Pamilya',
  'settings_about_dev_built_with': 'Ginawa gamit',
  'settings_dev_projects_title': 'Mga Proyekto',
  'settings_dev_projects_body':
      'Tingnan ang mga app at proyekto ni Arvel.',
  'settings_dev_for_nanay_title': 'Para kay Nanay',
  'settings_dev_for_nanay_body':
      'Isang espesyal na page para sa aming Nanay.',
  'settings_dev_project_memorial_body':
      'Isang trilingual na memorial para kay Lola Anita — ang kanyang kwento, pamilya, at mga larawan, na ingatan nang may pagmamahal.',
  'settings_contact_title': 'Makipag-ugnayan',
  'settings_contact_intro':
      'May nais ka bang ibahagi para kay Nanay? Magpadala ng mensahe, larawan, o kwento sa pamilya.',
  'settings_contact_name_label': 'Iyong pangalan',
  'settings_contact_name_hint': 'Ilagay ang iyong pangalan',
  'settings_contact_email_label': 'Email address',
  'settings_contact_email_hint': 'you@example.com',
  'settings_contact_message_label': 'Mensahe',
  'settings_contact_message_hint': 'Isulat ang iyong mensahe para sa pamilya…',
  'settings_contact_send_message': 'Mag-send ng mensahe',
  'settings_contact_opening': 'Binubuksan ang mail app…',
  'settings_contact_privacy': 'Ang iyong mensahe ay iingatan nang may pagmamahal.',
  'settings_contact_copied': 'Na-copy ang email address',
  'settings_search_hint': 'Maghanap ng setting...',
  'settings_search_empty': 'Walang nahanap na setting.',
  'candle_light': 'Sindihan ang kandila',
  'candle_virtual': 'Virtual na kandila na nakasindi sa kanyang alaala',
  'no_images':
      'Walang mga larawan sa galeri.\nSubukan ang buong pag-restart pagkatapos magdagdag ng mga larawan.',
  'story_quote': 'Marunong and Diyos',
  'story_quote_attribution': 'Nanay Nita, palaging sinasabi',
  'story_about':
      'Si Lola Anita Daiz Lumbao ay namuhay ng 85 taon na may biyaya, halakhak, at matibay na pananampalataya. '
      'Isa siyang mapagmahal na asawa, mapag-arugang ina, at puso ng pamilyang umaabot sa tatlong henerasyon. '
      'Laging abala ang kanyang mga kamay nagluluto, nananahi, o nakatiklop sa panalangin at ang kanyang tahanan ay laging bukas.\n\n'
      'Malalim ang kanyang paniniwala na ang pamilya ang pinakamahalagang yaman, at ibinigay niya ang lahat upang bumuo ng tahanang puno ng pagmamahal.',
  'timeline_birth_title':
      'Ipinanganak sa Barangay 7, Mercedes, Camarines Norte',
  'timeline_birth_desc':
      'Ikatlo sa pitong magkakapatid, ipinanganak sa probinsyang mahal niya.',
  'timeline_marriage_title': 'Napangaaswa kay Lolo Salvador Lumbao',
  'timeline_marriage_desc':
      '54 na taon ng pagsasama. Magkasama nilang pinalaki ang anim na anak at nagmahalan ng walang hanggan. '
      'Pumanaw si Lolo Salvador dahil sa atake sa puso.',
  'timeline_first_apo_title': 'Unang apo ay isinilang',
  'timeline_first_apo_desc': 'Isa syang naging ganap na lola.',
  'timeline_anniversary_title': 'Ipinagdiwang ang anibersaryo',
  'timeline_anniversary_desc':
      'Nagtipon ang buong pamilya upang parangalan ang 37 taon ng tapat na pagmamahalan.',
  'timeline_passing_title': 'Mapayapang namahinga',
  'timeline_passing_desc':
      'Pinalibutan ng pamilya, siya ay nagbalik sa Diyos matapos ang isang buhay na puno ng biyaya.',
  'memory_1_title': 'Ang Kanyang Tahanan',
  'memory_1_body':
      'Ang kanyang bahay ang puso ng pamilya, laging bukas, bawat sulok puno ng alaala. '
      'Doon nakatira ang kanyang init.',
  'memory_2_title': 'Ganyan si Nanay',
  'memory_2_body':
      'Maliit lang siya, pero taglay ang init ng tahanan. '
      'Sa ngiti at mga mata niya nakikita ang pagmamahal.',
  'memory_3_title': 'Halik ni Nanay',
  'memory_3_body':
      'Hinahalikan niya ang bawat apo sa noo, parang basbas. '
      'Sa halik na iyon natutong magmahal nang walang kapalit.',
  'memory_4_title': 'Hindi Namin Siya Pinabayaan',
  'memory_4_body':
      'Noong dumating ang sakit, kami ang humawak ng mga kamay niya. '
      'Nagbantay kami, hindi siya nag-isa hanggang dulo.',
  'memory_5_title': 'Matibay sa Gitna ng Sakit',
  'memory_5_body':
      'Kahit nanghihina ang katawan niya, hindi ipinakita ang pagkatalo. '
      'Lumaban siya para sa amin hanggang dulo.',
  'memory_6_title': 'Ang Kanyang Kaarawan',
  'memory_6_body':
      'Bawat kaarawan niya ay pista: kanta, handa, at tawanan. '
      'Ang ngiti niya sa araw na iyon ang paborito naming larawan.',
  'memory_7_title': 'Ang Kanyang Pagpanaw',
  'memory_7_body':
      'Tahimik siyang umalis: panloob na pagdurugo sa pancreas. '
      'Hindi siya nawala: nabubuhay sa tawanan, dasal, at tahanan namin.',
  'memories_subtitle': 'Mga sandaling hindi malilimutan',
  'memories_count': '{count} na alaala',
  'memories_photos_in_gallery': '{count} larawan sa Galeri',
  'mem_feature_title': 'Mga kuwentong pinagsasaluhan',
  'mem_feature_body':
      'Mga alaala, tawa, at aral ng buhay na inilaan at minana. '
      'Ang kanyang pag-ibig ang patuloy na gabay namin hanggang ngayon.',
  'mem_feature_quote':
      '\u201C Ang mga simpleng araw kasama siya ang pinakamatamis na alaala. \u2665 \u201D',
  'mem_open_photos': 'Buksan ang mga larawan',
  'mem_view_gallery': 'Tingnan sa Galeri',
  'mem_photo_count': '{count} na larawan',
  'family_name': 'Ang Pamilyang Lumbao',
  'family_subtitle': 'Mga taong bumuo ng kanyang mundo',
  'family_search_hint': 'Maghanap ng kapamilya',
  'family_search_results': '{count} natagpuang kapamilya',
  'family_search_empty': 'Walang nahanap na kapamilya.',
  'family_search_spouse_of': 'Asawa ni {name}',
  'family_filter_all': 'Lahat',
  'family_filter_direct': 'Direktang pamilya',
  'family_filter_apo': 'Mga Apo',
  'family_stat_members': '{count} miyembro ng pamilya',
  'family_stat_children': '{count} na anak',
  'family_stat_years': '{count} taon na magkasama',
  'family_stat_generations': '{count} henerasyon',
  'family_stat_siblings': '{count} magkakapatid',
  'family_intro_title': 'Ang pamilya ang kanyang pinakamalaking kayamanan',
  'family_intro_body':
      'Nagtayo siya ng tahanang puno ng pagmamahal, isang pamilyang umaabot '
      'sa tatlong henerasyon, na pinagbuklod ng kanyang init, pananampalataya, '
      'at pagluluto. Ang sinumang pumasok sa kanyang tahanan ay naging pamilya.',
  'family_role_husband': 'Asawa',
  'family_role_daughter': 'Panganay na anak na babae',
  'family_role_grandson': 'Apo',
  'family_role_granddaughter': 'Apo',
  'family_role_son': 'Anak na lalaki',
  'family_role_second_son': 'Ikalawang anak',
  'family_role_bunso': 'Bunso',
  'family_role_sister': 'Kapatid na babae',
  'family_role_brother': 'Kapatid na lalaki',
  'family_group_children': 'Mga Anak',
  'family_group_children_sub': 'anak',
  'family_group_siblings': 'Mga Kapatid',
  'family_group_siblings_sub': 'kapatid',
  'family_group_grandchildren': 'Mga Apo',
  'family_group_grandchildren_sub': 'miyembro',
  'family_group_nieces_nephews': 'Mga Pamangkin',
  'family_group_nieces_nephews_sub': 'pamangkin',
  'family_group_other_relatives': 'Iba pang Kamag-anak',
  'family_group_other_relatives_sub': 'miyembro',
  'family_root_subtitle': 'Sentro ng aming pamilya',
  'family_view_all_grandchildren': 'Tingnan ang lahat ng apo',
  'family_view_all_nieces_nephews': 'Tingnan ang lahat ng pamangkin',
  'family_view_all_relatives': 'Tingnan ang lahat ng kamag-anak',
  'family_members_word': 'miyembro',
  'family_photos_with': 'larawan kasama',
  'family_age_years': '{count} taong gulang',
  'family_age_months': '{count} buwan',
  'family_extra_count': '+{count} pa',
  'family_sheet_close': 'Isara',
  'family_sheet_about': 'Tungkol sa Kanya',
  'family_sheet_full_story': 'Buong Kuwento',
  'family_sheet_identity': 'Pagkakakilanlan',
  'family_sheet_nickname': 'Palayaw',
  'family_sheet_family': 'Pamilya',
  'family_sheet_role': 'Role',
  'family_sheet_birthplace': 'Pinagmulan',
  'family_sheet_occupation': 'Trabaho',
  'family_sheet_active_since': 'Aktibo mula noong',
  'family_sheet_spouse': 'Asawa',
  'family_sheet_children': 'Mga anak',
  'family_sheet_children_count': '{count} anak',
  'family_sheet_grandchildren': 'Mga apo',
  'family_sheet_grandchildren_count': '{count} na apo',
  'family_sheet_not_recorded': 'Walang naitala',
  'family_view_full_tree': 'Tingnan ang buong family tree',
  'family_footer_note':
      'Ang mga miyembro ng pamilya ay maaaring ikonekta sa mga alaala.',
  'family_member_salvador_bio':
      'Ang kanyang tapat na asawa. Limampu\'t apat na taon ng pagsasama, '
      'anim na anak na magkasamang pinalaki, at pagmamahal na hindi kumupas.',
  'family_member_maria_bio':
      'Ang kanyang panganay na anak na babae, na nagdadala ng kanyang init '
      'at karunungan.',
  'family_member_carlo_bio':
      'Ang kanyang apo, na natutunan na ang pagmamahal, hindi ang mga pader, '
      'ang nagtatayo ng tahanan.',
  'family_member_ana_bio':
      'Ang kanyang apo, na nabubuhay upang mahalin ang mga tao sa paraang '
      'minahal sila ni Lola.',
  'family_member_ramon_bio':
      'Ang kanilang panganay na anak na lalaki. Pinangalanan sa lakas na '
      'nakita niya sa bawat bagong simula.',
  'family_member_rosario_bio':
      'Ang kanilang anak na babae, na ang pangalan ay nangangahulugang rosaryo, '
      'isang dasal na sinagot.',
  'family_member_salvador_jr_bio':
      'Ang kanilang bunso na anak na lalaki, na nagdadala ng pangalan ng kanyang '
      'ama at ang puso ng kanyang ina.',
  'family_member_gernan_bio':
      'Ang kanilang panganay na anak na lalaki, ang pinakamasungit ngunit '
      'pinakamatapang sa pamilya, tapat na pintor at manggagawa, asawa nina '
      'Milagros, at ama ng dalawang anak.',
  'family_member_odin_bio':
      'Ang kanilang ikalawang anak na lalaki, ang pinakamadaling '
      'pakisamahan, mabait na magsasaka sa Camarines Sur, asawa nina '
      'Mersilita at ama ng dalawang anak.',
  'family_member_lorie_bio':
      'Ang kanilang bunso na anak na babae, ang pinakamamahal ni Nanay, '
      'matapang ngunit mapagkalinga, tapat na asawa nina Elben at ina ng '
      'apat na anak.',
  'family_member_ester_bio':
      'Ang kanyang kapatid, isang tahimik na kasama sa bawat panahon ng buhay.',
  'family_story_obit':
      'Si Roberto ang kilalang matapang sa mga kapatid nina Anita, handang '
      'ipaglaban ang pamilya sa oras ng pangangailangan. Sa likod ng tapang '
      'na iyan ay isang tapat at mapag-arugang asawa at ama. Kasama ni '
      'Josebeth, nakabuo sila ng pamilyang ngayon ay pinapasaya ng limang '
      'apo. Tahimik ang lakas ni Roberto: mapagkalinga sa mga mahal niya, '
      'bukas-palad sa oras, at laging handang tumulong, kaya\'t siya ang '
      'taong madaling sandigan ng buong pamilya.',
  'family_member_obit_bio':
      'Ang kanyang kapatid na lalaki, ang matapang, tapat na asawa nina '
      'Josebeth at ipinagmamalaking lolo ng limang apo.',
  'family_story_rodolfo':
      'Si Rodolfo ang pinakamasipag sa magkakapatid nina Anita. Mangingisda '
      'nang mahigit sampung taon, hindi pa rin siya nagreretiro at patuloy '
      'na nagbibigay sa sariling pamilya at tumutulong sa mga anak ng '
      'kapatid niyang si Lorie. Walang makapantay sa kaalaman niya sa '
      'dagat. Hindi mga aklat ang humubog sa karunungan niya kundi dekadang '
      'mabigat na trabaho, pasensya, at tahimik na pagmamahal sa mga taong '
      'minamahal niya.',
  'family_member_rodolfo_bio':
      'Ang kanyang kapatid na lalaki, mangingisda magpakailanman, asawa nina '
      'Lolita, ama ng tatlo, at lolo ng limang apo.',
  'family_story_sonia':
      'Si Sonia ang nag-iisang kapatid na babae nina Anita, kilala sa '
      'tapang at matibay na pananagutan. Naranasan niya ang malalim na '
      'pagkawala — pumanaw ang bunso niyang anak at asawa — ngunit dala '
      'niya ito nang may tahimik na lakas, napapaligiran ng nagmamahal '
      'niyang mga anak at apo, kabilang ang pinakabagong apo niyang si '
      'Zia. Bilang barangay tanod, siya ang bantay ng kanyang komunidad: '
      'matapang, masipag, at pusong laging nag-aalay sa pamilya at kapwa.',
  'family_member_sonia_bio':
      'Ang kanyang nag-iisang kapatid na babae, matapang at mapagkalinga, '
      'barangay tanod na nagbabantay sa komunidad, at lola ng tatlong apo.',
  'family_story_gernan':
      'Si Gernan ang panganay na anak nina Anita, kilala bilang '
      'pinakamasungit ngunit siya rin ang pinakamatapang, hindi umaatras '
      'kapag may kailangang ipagtanggol. Sa likod ng matigas na anyo ay '
      'tapat na asawa at amang buong pusong nagtrabaho kina Milagros at '
      'sa kanilang dalawang anak. Bilang pintor at manggagawa, hanggang '
      'Baguio umabot ang galing niya. Mahigpit at masinop siya sa trabaho '
      'at kapaligiran — tanda ng pagmamalaki sa bawat ginagawa niya.',
  'family_story_odin':
      'Si Rodel ang pinakamadaling pakisamahan sa magkakapatid, ang laging '
      'nagdadala ng tawanan kahit saan magpunta. Kasama ang asawang si '
      'Mersilita, pinalaki niya ang dalawang anak na parehong tapos na sa '
      'kolehiyo, at may bagong apo na rin siya. Magsasaka siya sa '
      'Camarines Sur na masipag mag-alaga ng lupa. Tuwing pista, '
      'sinisiguro niyang napupuntahan si Nanay Anita na may dalang bigas '
      'at itlog — simpleng patunay ng pagkatao niya: mapagbigay, '
      'maaasahan, at pamilya muna.',
  'family_story_lorie':
      'Si Lorie ang bunso at pinakamamahal ni Nanay Anita, siyang '
      'nag-alaga rito nang walang sawa hanggang sa huling sandali. '
      'Nakapangasawa siya ni Elben Salvador, at magkasama silang '
      'nagpalaki ng apat na talinong anak kabilang si Arvel, ang '
      'developer ng app na ito. May pambihira siyang halo ng lakas at '
      'lambing: sapat ang tapang upang harapin ang mga pagsubok ng '
      'buhay, mahinahon ang puso sa bawat taong minamahal niya. Ang '
      'paglilingkod niya sa ina, asawa, at anak ay salamin ng pagmamahal '
      'na hindi nauubos.',
  'family_member_hanna_bio':
      'Anak ni Gernan Lumbao. Masayahin at puno ng enerhiya, sa bawat '
      'silid na pinapasok niya, parang nagliwanag ito, at napapasaya '
      'niya ang buong pamilya.',
  'family_member_audrey_bio':
      'Anak ni Gernan Lumbao. Tahimik ngunit may kumpiyansa, may '
      'estilo siyang sarili niya, at hindi natatakot na maging ang '
      'tunay niyang sarili.',
  'family_member_jongjong_bio':
      'Anak ni Rodel Lumbao Sr.. Makulit at madaling pakisamahan, mana sa '
      'init ng loob ng kanyang tatay, at alam niyang magpatawa ng '
      'kahit sino.',
  'family_member_roseann_bio':
      'Anak ni Rodel Lumbao Sr.. Maamo at mahinahon magsalita, dala niya '
      'ang banayad at nakakakalma na presensya sa bawat pagtitipon ng '
      'pamilya.',
  'family_member_arvel_bio':
      'Anak ni Lorie Salvador. Ang techie ng pamilya, laging mausisa '
      'at masikap, at siya ang mismong developer ng app na ito na '
      'nagpapanatiling buhay ang kuwento ng pamilya.',
  'family_member_aivan_bio':
      'Anak ni Lorie Salvador. Kalmado at masipag, siya ang '
      'maaasahang anak na laging sandigan ng pamilya.',
  'family_member_honey_bio':
      'Anak ni Lorie Salvador. Palakaibigan at puno ng buhay, laging '
      'siya ang unang nagdadala ng enerhiya at excitement sa bawat '
      'pagtitipon.',
  'family_member_daniel_bio':
      'Anak ni Lorie Salvador. Matatag at mabait ang puso, may tahimik '
      'na pagkamature, at mapag-aruga sa mga taong nasa paligid niya.',
  'family_story_hanna':
      'Si Hanna Mae ay anak ni Gernan Lumbao. Masayahin at puno ng '
      'enerhiya, parang nagliwanag ang bawat silid na pinapasok niya. '
      'Napapasaya niya ang buong pamilya sa kanyang ngiti at kulit, at '
      'sa bawat pagtitipon siya ang maliit na ilaw na hindi nauubusan '
      'ng kwento at tawa.',
  'family_story_audrey':
      'Si Audrey ay anak ni Gernan Lumbao. Tahimik ngunit may '
      'kumpiyansa, may sarili siyang estilo at pananaw na hindi '
      'natatakot ipakita. May lakas sa kanyang katahimikan, at sa bawat '
      'hakbang niya ipinapakita niyang ang tunay na sarili ang '
      'pinakamagandang suot.',
  'family_story_jongjong':
      'Si Rodel Lumbao Jr. ay anak ni Rodel Lumbao Sr.. Makulit at madaling '
      'pakisamahan, minana ang init ng loob ng kanyang tatay. Alam '
      'niyang magpatawa kahit sino, at sa bawat pagtitipon siya ang '
      'nagpapasiklab ng tawanan — laging handang makipagkulitan sa mga '
      'pinsan.',
  'family_story_roseann':
      'Si Rose-ann ay anak ni Rodel Lumbao Sr.. Maamo at mahinahon magsalita, '
      'dala niya ang banayad at nakakakalma na presensya sa bawat '
      'pagtitipon. Sa kanyang lambing at paggalang, ipinapakita niyang '
      'ang tahimik na kabaitan ang pinakamataas na anyo ng pagmamahal '
      'sa pamilya.',
  'family_story_arvel':
      'Si Arvel ay anak ni Lorie Salvador. Ang techie ng pamilya: laging '
      'mausisa, masipag, at puno ng ideya. Siya ang mismong developer '
      'ng app na ito, ang nagpapanatiling buhay ang mga kuwento at '
      'alaala ng pamilya. Pagkatapos ng kolehiyo, patuloy siyang '
      'naglilingkod sa pamilya sa kanyang galing.',
  'family_story_aivan':
      'Si Aivan ay anak ni Lorie Salvador. Kalmado at masipag, siya ang '
      'maaasahang anak na laging sandigan ng pamilya. Tahimik man sa '
      'umpisa, malalim naman ang pagmamahal: tinatapos niya nang maayos '
      'ang bawat gawain, at laging handang tumulong nang walang '
      'sabi-sabi.',
  'family_story_honey':
      'Si Honey ay anak ni Lorie Salvador. Palakaibigan at puno ng '
      'buhay, siya ang unang nagdadala ng enerhiya at kasiyahan sa '
      'bawat pagtitipon. Sa kanyang tawa at kwento, dala niya ang init '
      'ng matinong tahanan na kanyang kinagisnan mula kay Lola.',
  'family_story_daniel':
      'Si Daniel ay anak ni Lorie Salvador. Matatag at mabait ang puso, '
      'may tahimik na pagkamature na higit sa kanyang edad. Mapag-aruga '
      'siya sa mga nakapalibot sa kanya, at sa bawat hakbang ng buhay '
      'ipinapakita niya ang disiplina at pagmamahal na itinuro ng '
      'pamilya.',
  'tribute_message':
      'Itinuro mo sa amin na ang tahanan ay itinatayo sa init ng pagmamahal, hindi sa mga pader. '
      'Ang iyong tawa ay nananatili sa bawat silid na minahal namin. '
      'Magpahinga ka na, Lola. Patuloy ka naming dinadala sa aming mga puso.',
  'tribute_until_we_meet': 'Hanggang sa muli tayong magkita',
  'tribute_in_loving_memory': 'SA MAHAL NA ALAALA',
  'tribute_subtitle': 'Huling paalam mula sa pamilyang kanyang minahal',
  'family_quote_1':
      'Si Mama mismo ang liwanag. Parang sumisigla ang bawat silid na kanyang pinasukan.',
  'family_quote_1_name': 'Maria, ang panganay niyang anak na babae',
  'family_quote_2':
      'Hindi niya kami hinayaang umalis na gutom o walang pagmamahal. Iyon ang kanyang superpower.',
  'family_quote_2_name': 'Carlo, apo',
  'family_quote_3':
      'Gugugulin ko ang buong buhay ko sa pagsisikap na mahalin ang mga tao sa paraang minahal niya kami.',
  'family_quote_3_name': 'Ana, apo',
  // ── Words tab ─────────────────────────────────────────────────────────
  'words_subtitle':
      'Mga salita ng pagmamahal ng pamilya para kay Nanay',
  'words_leave_cta': 'Mag-iwan ng salita',
  'words_filter_all': 'Lahat',
  'words_filter_children': 'Mga anak',
  'words_filter_grandchildren': 'Mga apo',
  'words_filter_siblings': 'Mga kapatid',
  'words_featured_label': 'PINAKAMINAMAHAL NA ALAALA',
  'words_featured_quote':
      'Hindi mo man lagi sinasabi, ramdam namin ang pagmamahal mo sa bawat araw ng aming buhay.',
  'words_featured_name': 'Pamilya',
  'words_featured_relation': 'Salita ni Lorie Salvador para kay Nanay',
  'words_date_1': 'Naiwan noong May 16, 2025',
  'words_date_2': 'Naiwan noong May 15, 2025',
  'words_date_3': 'Naiwan noong May 14, 2025',
  'words_date_4': 'Naiwan noong May 13, 2025',
  'words_date_5': 'Naiwan noong May 12, 2025',
  'words_date_6': 'Naiwan noong May 11, 2025',
  'words_date_7': 'Naiwan noong May 10, 2025',
  'words_date_8': 'Naiwan noong May 9, 2025',
  'words_date_9': 'Naiwan noong May 8, 2025',
  'words_date_10': 'Naiwan noong May 7, 2025',
  'words_date_11': 'Naiwan noong May 6, 2025',
  'words_date_12': 'Naiwan noong May 5, 2025',
  'words_date_13': 'Naiwan noong May 4, 2025',
  'words_date_14': 'Naiwan noong May 3, 2025',
  'words_relation_bunso': 'Bunso',
  'words_relation_panganay': 'Panganay',
  'words_relation_ikalawa': 'Ikalawang anak',
  'words_relation_kapatid_lalaki': 'Kapatid na lalaki',
  'words_relation_kapatid_babae': 'Kapatid na babae',
  'words_relation_apo_lalaki': 'Apo na lalaki',
  'words_relation_apo_babae': 'Apo na babae',
  // Mga anak
  'words_quote_1':
      'Salamat sa pag-aaruga, Nay. Sa bawat "mahal kita" ay isang hakbang na hindi kailanman kulang.',
  'words_quote_1_name': 'Lorie Salvador',
  'words_quote_2':
      'Buong pusong nagtrabaho ka para sa amin. Magpahinga ka na, Nay.',
  'words_quote_2_name': 'Gernan Lumbao',
  'words_quote_3':
      'Hindi man kita nadakotup ngayon, dala-dala ko pa rin ang mga kuwento at tawanan natin.',
  'words_quote_3_name': 'Rodel Lumbao Sr.',
  // Mga kapatid
  'words_quote_4':
      'May isang sulok sa bawat sulok ng bahay at sa puso naming lahat.',
  'words_quote_4_name': 'Roberto Daiz',
  'words_quote_5':
      'Ang dagat ang nagturo sa akin ng pasensya, ikaw ang nagturo ng puso.',
  'words_quote_5_name': 'Rodolfo Daiz',
  'words_quote_6':
      'Ikaw ang nagdasal para sa amin. Ngayon, kami naman ang magdadasal para sa iyo.',
  'words_quote_6_name': 'Sonia Daiz',
  // Mga apo
  'words_quote_7':
      'Mas masaya bawat pagtitipon dahil ikaw ang sinimulan ng liwanag ng pamilya namin, Lola.',
  'words_quote_7_name': 'Hanna Lumbao',
  'words_quote_8':
      'Itinuro mo na ang pagiging totoo sa sarili ang pinakamagandang suot.',
  'words_quote_8_name': 'Audrey Lumbao',
  'words_quote_9':
      'Ang tawa mo ang tugtog ng aming pagkabata. Patuloy naming pinapatugtog, Lola.',
  'words_quote_9_name': 'Rodel Lumbao Jr.',
  'words_quote_10':
      'Ang lambing mo ay nabubuhay sa paraan ng aming pananalita. Salamat, Lola.',
  'words_quote_10_name': 'Rose-ann Lumbao',
  'words_quote_11':
      'Ang app na ito ang nagpapanatiling buhay ang mga kuwento mo. Alay ko ito sa iyo, Lola.',
  'words_quote_11_name': 'Arvel Salvador',
  'words_quote_12':
      'Tahimik ngunit tiyak, ipinakita mo kung paano tapusin ang mga bagay nang may pagmamahal.',
  'words_quote_12_name': 'Aivan Salvador',
  'words_quote_13':
      'Bawat pista, bawat kaarawan — ang kasiyahan mo ang recipe naming sinusundan.',
  'words_quote_13_name': 'Honey Salvador',
  'words_quote_14':
      'Tinawag mo kami sa aming mga pangalan at ginawa mo kaming nararamdaman na tayo ay kapansin-pansin.',
  'words_quote_14_name': 'Daniel Salvador',
  'candle_lit': 'Maraming Salamat',
  'candle_thanks_subtitle':
      'Sa lahat ng nakikiramay at nagmamahal kay Nanay Nita.',
  'candle_thank_you': '🕊️ Salamat',
  'candle_message_title': 'Anong gusto mong sabihin kay Nanay?',
  'candle_message_hint': 'Isulat ang iyong mensahe para kay Nanay…',
  'candle_message_send': 'Ipadala',
  'candle_message_thanks': 'Nakarating kay Nanay ang iyong mensahe 🕊️',
  'candle_lit_times': '{count} kandila na ang sinindihan',
  'candle_condolences_button': 'Pakikiramay',
  'condolences_keepsake': 'Ang alaala ay patuloy na nagliliwanag',
  'condolences_each_flame': 'Sa bawat sindi, may alaala.',
  'group_celebrations': 'Mga Pagdiriwang',
  'group_bahay': 'Bahay',
  'group_family': 'Pamilya',
  'group_care': 'Pag-aalaga',
  'group_gatherings': 'Pagkain',
  'group_portraits': 'Mga Retrato',
  'group_remembrances': 'Huling Araw',
  'group_other': 'Iba Pa',
  'gallery_all': 'Lahat',
  'gallery_subtitle': 'Mga alaala na patuloy na nabubuhay',
  'gallery_highlights': 'Tampok',
  'gallery_highlights_title': 'Mga Tampok na Alaala',
  'gallery_curated_memory': 'Piniling alaala',
  'gallery_play_all': 'I-play lahat',
  'gallery_photos': 'larawan',
  'gallery_all_photos_label': 'Lahat ng Larawan',
  'gallery_search_hint': 'Maghanap ng litrato',
  'gallery_clear_all': 'I-clear lahat',
  'gallery_tap_to_reveal': 'Pindutin upang makita',
  'gallery_choose_music': 'Pumili ng Musika',
  'remembrance_gate_label': 'Sa Pag-alaala',
  'remembrance_gate_title': 'Sa kanyang huling mga araw, hindi siya nag-iisa.',
  'remembrance_gate_subtitle':
      'Pindutin ang kandila upang sindihan ito bilang alaala sa kanya.',
  'remembrance_gate_lit': 'Ang kanyang alaala ay patuloy na nabubuhay sa atin.',
  'remembrance_gate_held': ', itinuring nang may pagmamahal.',
  'remembrance_gate_tap_hint': 'Pindutin kahit saan upang sindihan ang kandila',
  'remembrance_gate_waiting': 'Unti-unting lilitaw ang kanyang mga larawan...',
  'remembrance_gate_please_tap': 'Pindutin po',
  'loc_lipa': _address,
  'loc_bahay': _address,
  'loc_family_residence': _address,
  'loc_batangas_province': _address,
  'date_1':
      'Marso 12, 2018', // TODO: label which gallery photo/event this date belongs to
  'date_2':
      'Agosto 21, 2011', // TODO: label which gallery photo/event this date belongs to
  'date_3':
      'Mayo 14, 2019', // TODO: label which gallery photo/event this date belongs to
  'date_4':
      'Disyembre 25, 2020', // TODO: label which gallery photo/event this date belongs to
};
